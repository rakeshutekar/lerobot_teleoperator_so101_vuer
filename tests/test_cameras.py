...
def test_camera_names_are_the_contract():
    assert CAMERA_NAMES == ("wrist", "front", "overhead")


def test_build_cameras_arg_emits_every_camera_with_format():
    arg = build_cameras_arg({"front": 0, "overhead": 1, "wrist": 2})
    for name, index in (("front", 0), ("overhead", 1), ("wrist", 2)):
...
def test_build_cameras_arg_accepts_device_paths():
    arg = build_cameras_arg({"front": "/dev/video0", "overhead": 1, "wrist": 2})
...
def test_build_cameras_arg_rejects_wrong_names():
    with pytest.raises(ValueError, match="wrist"):
        build_cameras_arg({"front": 0, "overhead": 1})
    with pytest.raises(ValueError, match="side"):
        build_cameras_arg({"wrist": 0, "front": 1, "overhead": 2, "side": 3})


def test_load_camera_map_reads_json(tmp_path):
    path = tmp_path / "cameras.json"
    path.write_text(json.dumps({"front": 0, "overhead": 1, "wrist": 2}))
    assert load_camera_map(path) == {"front": 0, "overhead": 1, "wrist": 2}
